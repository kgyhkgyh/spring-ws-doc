...
```
* (1) HolidayEndpoint加上了@Endpoint注释，这个标记使得它变成了一种特殊的Sping里的@Component，从而使它可以处理
XML消息，从而Spring可以进行注入。
* (2) HolidayEndpoint需要用到HuamnResourceService来操作，因此我们通过@Autowired注入。下一步，我们用到JDOM2的API 来建立XPath表达式： //hr:StartDate用来抽取<StartDate>的标签内容，以此类推
* (3) @PayloadRoot注释告诉Spring-ws handleHolidayRequest方法是用来处理XML消息，它能够处理的消息类型也在注释中表明。
* (4) handleHolidayRequest方法是真正用来处理XMl消息的，
